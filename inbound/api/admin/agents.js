/**
 * api/admin/agents.js — Inbound agent controls
 *
 * Exports:
 *   buildAgentControlsHtml(agents) — renders the "Agent Controls" card
 *   registerAgentRoutes(router)    — POST routes (mounted on inbound adminRouter)
 *   startInboundScheduler()        — 60-second scheduler
 */
...
import { requireAuth, esc, adminShell } from './auth.js';

const __dirname   = path.dirname(fileURLToPath(import.meta.url));
const INBOUND_DIR = path.resolve(__dirname, '../..');

const AGENT_SCRIPTS = {
  'inbound.discovery':    path.join(INBOUND_DIR, 'scripts/run-discovery.js'),
  'inbound.site_factory': path.join(INBOUND_DIR, 'scripts/run-site-factory.js'),
  'inbound.traffic':      path.join(INBOUND_DIR, 'scripts/run-traffic.js'),
  'inbound.optimizer':    path.join(INBOUND_DIR, 'scripts/run-optimizer.js'),
};
...
// ─── Status badge (uses inbound CSS) ─────────────────────────────────────────

function runBadge(status, alwaysOn) {
  if (alwaysOn) return `<span class="badge bg">Always On</span>`;
  const cls = { success: 'bg', failed: 'br', running: 'bb' };
  const c = status ? (cls[status] || 'bk') : 'bk';
  return `<span class="badge ${c}">${esc(status || 'Never run')}</span>`;
}
...
function agentRow(a) {
  ...
  const toggleBtn = a.always_on
    ...
    : `<form method="POST" action="/admin/inbound/agents/${a.id}/toggle" style="display:inline">
         <button class="btn btn-sm ${a.enabled ? 'btn-green' : 'btn-red'}">${a.enabled ? 'Enabled' : 'Disabled'}</button>
       </form>`;

  const runBtn = a.always_on ? '' :
    `<form method="POST" action="/admin/inbound/agents/${a.id}/run" style="display:inline">
       <button class="btn btn-sm btn-dark" ${!canRun ? 'disabled style="opacity:.4;cursor:not-allowed"' : ''}>${isRunning ? '⟳ Running…' : 'Run Now'}</button>
     </form>`;

  return `
<div style="padding:14px 22px;border-bottom:1px solid #f0ede8;display:flex;align-items:center;gap:14px;flex-wrap:wrap">
  <div style="flex:1;min-width:150px">
    <div style="font-family:'Outfit',sans-serif;font-size:14px;font-weight:600;color:#1a1a1a">${esc(a.name)}</div>
    <div class="ph-sub" style="font-size:12px;margin-top:2px">${esc(a.description || '')}</div>
  </div>
  ...
  <form method="POST" action="/admin/inbound/agents/${a.id}/schedule" style="display:flex;align-items:center;gap:6px">
    <select name="schedule" class="form-i" style="width:auto;padding:6px 10px;font-size:13px" ${a.always_on ? 'disabled' : ''}>
      ${scheduleOptions(scheduleKey(a))}
    </select>
    ${a.always_on ? '' : `<button type="submit" class="btn btn-sm btn-ghost">Save</button>`}
  </form>
...
const PREFIX = '/admin/inbound/agents';

function compactScheduleSelect(a) {
...
function compactPill(a) {
  if (a.always_on) {
    return `<div style="display:flex;align-items:center;gap:4px;border:1px solid #bbf7d0;background:#f0fdf4;border-radius:100px;padding:4px 10px 4px 7px">
      ...
      <span style="font-family:'Outfit',sans-serif;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:#16a34a;white-space:nowrap">${esc(a.name)}</span>
    </div>`;
  }
  ...
  return `<div style="display:flex;align-items:center;gap:3px;background:#f5f3ef;border:1px solid #e8e5e0;border-radius:100px;padding:4px 10px 4px 7px">
    ...
    <span style="font-family:'Outfit',sans-serif;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;color:${nameColor};margin:0 2px;white-space:nowrap">${esc(a.name)}</span>
    ${compactScheduleSelect(a)}
...
export function buildCompactAgentBar(agents) {
  const pills = (agents || []).map(a => compactPill(a)).join('');
  return `<div style="display:flex;align-items:center;gap:5px;margin-left:auto;padding-left:16px;flex-shrink:0">${pills}</div>`;
}

// ─── Full agent controls card HTML ────────────────────────────────────────────
...
<div class="card">
  <div class="ch">
    <span class="ct">Agent Controls</span>
    <div style="display:flex;gap:8px">
      <form method="POST" action="/admin/inbound/agents/all/on" style="display:inline">
        <button class="btn btn-sm btn-green">Turn All On</button>
      </form>
      <form method="POST" action="/admin/inbound/agents/all/off" style="display:inline">
        <button class="btn btn-sm btn-red">Turn All Off</button>
      </form>
    </div>
  </div>
  ${rows || `<div class="empty">No agents configured.</div>`}
</div>`;
}
...
async function runAgent(agentId) {
  ...
  child.on('close', async code => {
    ...
    console.log(`[agents:inbound] ${agentId} → ${status}`);
  });
...
export async function startInboundScheduler() {
  await supabase.from('agent_states')
    ...
    .eq('system', 'inbound').eq('last_run_status', 'running');

  setInterval(async () => {
    const { data: agents } = await supabase.from('agent_states').select('*').eq('system', 'inbound');
    const now = new Date();
    for (const a of agents || []) {
      if (shouldRun(a, now)) {
        console.log(`[scheduler:inbound] triggering ${a.id}`);
        runAgent(a.id).catch(err => console.error(`[scheduler:inbound] ${a.id}:`, err.message));
      }
    }
  }, 60_000);

  console.log('[agents:inbound] Scheduler started.');
}
...
export function registerAgentRoutes(router) {
  router.get('/agents', requireAuth, async (_req, res) => {
    const { data: agents } = await supabase.from('agent_states').select('*').eq('system', 'inbound').order('id');
    const body = `
<div class="ph">
  <div class="ph-left"><h1><em>Agent Controls.</em></h1>
    <div class="ph-sub">Manage inbound agent schedules and trigger manual runs.</div></div>
</div>
${buildAgentControlsHtml(agents || [])}`;
    res.send(adminShell('Agent Controls', body, '/admin/inbound/agents'));
  });

  const back = (req) => req.get('Referer') || '/admin/inbound';

  router.post('/agents/all/on', requireAuth, async (req, res) => {
    await supabase.from('agent_states')
      .update({ enabled: true, updated_at: new Date().toISOString() })
      .eq('system', 'inbound').eq('always_on', false);
